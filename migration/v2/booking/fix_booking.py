...
def changeProperty(df):
	...
	'Taman Asih':'PR0304',\
	'Tamu Ibu':'PR0305',\
	'Bingin Garden':'PR0306',\
	'Breezy Point':'PR0307'})
	return df

def changeEmployeeProfile(df):
	dfEP = pd.DataFrame(df.employee_id.str.split('-',1).tolist(),columns = ['employee_id','profile_id'])
	dfEP['employee_id']=dfEP['employee_id'].map({
		'NULL':'E0000',\
		'Air Support 1':'E0001',\
		'Air Support 2':'E0002',\
		'Bayu':'E0003',\
		...
		'Dareiga':'E0026',\
		'Jack':'E0027'})
	dfEP['profile_id']=dfEP['profile_id'].map({
		'NULL':'PA0000',\
		'W':'PA0001',\
		'BV':'PA0002',\
		'BC':'PA0003',\
...
df = pd.read_csv('b_regint.csv')
df['extra'] = df.extra.astype(str)
df['property_id'] = df.property_id.astype(str)
dfProp = pd.DataFrame(df.property_id.str.split('#',1).tolist(),columns = ['property_id','extra'])
dfProp = pd.DataFrame(dfProp.property_id.str.split('-',1).tolist(),columns = ['property_id','extra'])
#dfProp.to_csv('res.csv')
dfProp=changeProperty(dfProp)
df['booking_check_in'] = pd.to_datetime(df.booking_check_in).dt.strftime('%Y-%m-%d')
df['booking_check_out'] = pd.to_datetime(df.booking_check_out).dt.strftime('%Y-%m-%d')
df['booking_received'] = pd.to_datetime(df.booking_received).dt.strftime('%Y-%m-%d %H:%M:%S')
df['canceled'] = pd.to_datetime(df.canceled).dt.strftime('%Y-%m-%d %H:%M:%S')
dfEP=changeEmployeeProfile(df)
dfPr=dfProp['property_id'].fillna('PR0000')
dfB=df['booking_code'].fillna('NULL')
dfP=dfEP['profile_id'].fillna('PA0000')
dfE=dfEP['employee_id'].fillna('E0000')
cols=['booking_received','booking_guest_name']
cols2=['booking_status','property_name']
cols3=['booking_check_in','booking_check_out','booking_guest_number','booking_earned','booking_currency','booking_guest_phone','canceled']
dfNew=pd.DataFrame(df,columns=cols)
dfNew2=pd.DataFrame(df,columns=cols2)
dfNew3=pd.DataFrame(df,columns=cols3)
df1=pd.concat([dfNew,dfB,dfNew2,dfPr,dfNew3,dfP,dfE],axis=1)
df1.index = np.arange(1, len(df1)+1)
df1.to_csv("legacy_regint.csv")
#